...
# MODIFIED BY Jibirl B. Coulibaly @ Northwestern University to not use the cull() function and not delete the duplicates
# This is useful when we want to print out some data using the run 0 because the timestep is identical and the regular log.py deletes it with the cull() function
...
class log_duplicate:
  ...
  def read_all(self):
    ...
    #self.cull() # MODIFICATION: Duplicates are not deleted upon reading, the would be managed separately during post-processing
